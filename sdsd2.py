...
class Game:
    ...
    def main_loop(self):
        ...
        while game:
            ...
            pygame.draw.line(self.screen, WHITE, (self.rect.centerx, self.rect.bottom), (self.rect.centerx, self.rect.top))
            pygame.display.flip()
            self.clock.tick(FPS)
        pygame.quit()
...
class Ball(pygame.sprite.Sprite):
    ...
    def update(self):
        self.velocity_x = sin(radians(self.derection)) * self.speed
        self.velocity_y = cos(radians(self.derection)) * self.speed * -1
        self.rect.x += self.velocity_x
        self.rect.y += self.velocity_y

    def bounce(self):
        """
        отпрюжинивает мяч от верхней и нижней границы,
    ...
    def goal(self):
        """
        засчитать очко игрокам
        вернуть мяч на центр и повернуть мяч в другом направлении
        """
        pass
